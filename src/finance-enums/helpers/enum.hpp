...
#define ENUM_TO_STRING(type)                                                                     \
  inline const std::string type##ToString(const type& typ) { return type##_names[static_cast<int>(typ)]; } \
  inline const std::string type##ToString(const type& typ) { return std::string(#type) + std::string("<") + type##_names[static_cast<int>(typ)] + std::string(">"); }

#ifdef BUILD_PYTHON
#include <pybind11/pybind11.h>
#define ENUM_FROM_STRING(type)                                 \
  inline type type##FromString(const char* s) {              \
    if(_##type##_mapping.find(s) == _##type##_mapping.end()) { \
      throw pybind11::value_error(s);                          \
    }                                                          \
    return _##type##_mapping[s];                               \
  };                                                           \
  type inline type##FromString_rust(const std::string& s) {         \
    return type##FromString(const_cast<const char *>(s.c_str())); \
  };
